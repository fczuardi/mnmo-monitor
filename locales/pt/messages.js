export default {
    locale: 'pt-BR',
    network: {
        loadingData: 'Carregando dados… Aguarde.'
    },
    splash: {
        appName: 'app.name.here'
    },
    login: {
        welcome: 'Bem vindo',
        username: 'usuário',
        password: 'senha',
        saveInfo: 'salvar informações',
        iAgree: 'Li e concordo: {tosLink}',
        tos: 'Termos de uso',
        submit: {
            loading: 'Carregando…',
            missingUsername: 'Preencha seu usuário e senha',
            missingPassword: 'Preencha seu usuário e senha',
            missingTos: 'Precisa concordar para usar',
            missingCaptcha: 'Já respondeu a pergunta?',
            access: 'Acessar'
        },
        loadingCaptcha: 'Carregando a pergunta…',
        question: 'Quanto é {captchaChallenge} ?'
    },
    password: {
        newPasswordTitle: 'Redefinir senha',
        forgotPasswordLink: 'Esqueci minha senha',
        forgotPasswordTitle: 'Esqueceu a senha?',
        email: 'email',
        changePassword: 'Alterar senha',
        currentPassword: 'senha atual',
        newPassword: 'nova senha',
        confirmNewPassword: 'confirmar nova senha',
        submit: {
            validatingToken: 'Aguarde…',
            change: 'Alterar',
            send: 'Enviar'
        },
        cancel: 'Cancelar'
    },
    settings: {
        menu: 'Menu',
        autoUpdate: 'Autoupdate',
        autoUpdateStatus:'AUTO UPDATE',
        columns: 'Colunas',
        close: 'Fechar',
        save: 'Salvar',
        back: 'Voltar',
        colors: 'Cores',
        tos: 'Termos de serviço',
        help: 'Ajuda',
        print: 'Imprimir',
        setPrintInterval: 'Definir intervalo',
        changePassword: 'Alterar senha',
        logout: 'Sair'
    },
    calendar: {
        weekdaysLong: ['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado'],
        weekdaysShort: ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
        months: ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro']
    },
    base: {
      column: 'Horário base: lado a lado',
      row:  'Horário base: empilhado'
    },
    chart: {
        onButton: 'Gráfico',
        offButton: 'Vazio',
        pieChartLabel: 'Esse gráfico representa apenas as emissoras selecionadas e não inclui outras audiências'
    },
    groups: {
        title: 'Grupos',
        type1: 'Tipo 1',
        type2: 'Tipo 2'
    },
    vars: {
        title: 'Variáveis',
        input1: 'Primária',
        input2: 'Secundária',
        displaySecondary: 'Exibir variável secundária na tabela'
    },
    error: {
        error: 'Erro',
        ok: 'OK',
        tryAgain: 'Tentar Novamente',
        invalidPasswordLink: 'Link para troca de senha inválido',
        warning: 'Aviso'
    },
    rows: {
        title: {
            list: 'Linhas',
            merged: 'Linhas Agrupadas',
        },
        date: 'Data',
        datePlaceholder: 'AAAA-MM-DD',
        hour: 'Hora',
        minute: 'Minuto',
        startingTime: 'Hora Inicial',
        endingTime: 'Hora Final',
        interval: 'Periodicidade (em minutos)',
        secondTable: 'Comparar',
        mergeRows: 'Agrupar linhas',
        unmergeRows: 'Ver todas',
        range: 'Período',
        headerPrint: {
            minute: 'MIN',
            base: 'BASE',
        },
    },
    subgroups: {
        title: 'Sub-grupos',
        subgroup: 'Sub-grupo'
    },
    classes: {
        title: 'Classe'
    }
};
